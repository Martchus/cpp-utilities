...
if (NOT META_NO_INSTALL_TARGETS AND ENABLE_INSTALL_TARGETS)
    ...
    set(PACKAGE_NAME "${NAMESPACE_PREFIX}${META_PROJECT_NAME}${META_CONFIG_SUFFIX}")
    set(EXPORT_NAME "${PACKAGE_NAME}Targets")
    set(EXPORT_NAME_CPS "${PACKAGE_NAME}-cps")
    install(
    ...
    if (CMAKE_VERSION VERSION_GREATER_EQUAL "4.3")
        install(
            TARGETS ${TARGETS_TO_EXPORT}
            EXPORT "${EXPORT_NAME_CPS}"
            RUNTIME DESTINATION ${CMAKE_INSTALL_BINDIR} COMPONENT binary
            LIBRARY DESTINATION ${LIBRARY_DESTINATION} COMPONENT binary
            ARCHIVE DESTINATION ${LIBRARY_DESTINATION} COMPONENT binary)
        install(PACKAGE_INFO "${PACKAGE_NAME}" EXPORT "${EXPORT_NAME_CPS}"
            PROJECT "${META_PROJECT_NAME}"
            LOWER_CASE_FILE
            VERSION "${META_VERSION_MAJOR}.${META_VERSION_MINOR}.${META_VERSION_PATCH}"
            COMPAT_VERSION "${META_SOVERSION}"
            VERSION_SCHEMA "simple"
            DEFAULT_TARGETS "${META_TARGET_NAME}"
            LICENSE "${META_PROJECT_LICENSE}"
            DEFAULT_LICENSE "${META_PROJECT_LICENSE}"
            DESCRIPTION "${META_APP_DESCRIPTION}"
            HOMEPAGE_URL "${META_APP_URL}"
            COMPONENT cps-config)
    endif ()
