...
This example is rather generic. For a development build, I recommend using CMakePresets as
...
Its setup wizard allows you to install the SDK and other useful tools. The Gradle project files created by Qt can
...
The open-source version is sufficient, but a Qt account is required. Alternatively, you can also skip this
...
required to build Syncthing Tray, as the use of libiconv, Boost, OpenSSL, CppUnit, and system SQLite3 is optional (so
...
After following the [instructions for importing my GPG key](https://martchus.dyn.f3l.de/repo/arch/ownstuff), you
...
Whether builds using any of these libraries will actually work at runtime has not been tested yet.
</details>
...
* `QT_ANDROID_KEYSTORE_PATH`: path of the directory containing Android keystores
...
Then the build can be conducted in an MSYS2 shell, e.g.:
