...
Qt one also needs the Android SDK and Qt for Android.
...
Additional libraries can be installed via MSYS2 using my Arch Linux packaging. Note that this is not required to
build Syncthing Tray as use of libiconv, Boost, OpenSSL and CppUnit is optional (so only Qt is required besides
the C/C++ standard libraries). To install additional libraries via MSYS2, add my Arch Linux repository to
`/etc/pacman.conf`:
